// Fejgep nelkuli teszt a gamepad.js-hez: minimalis DOM-utanzat + hamis padek.
// Igy a padkezeles bongeszo nelkul is vegigmerheto.
...
const M = await import('../v3/src/gamepad.js');
...
// --------------------------------------------------------------- 13. input.js
// A jatek oldali forditas: iranyok jatekosonkent, a tobbi gomb kozos.
// A slotkiosztas az elozo lepesek utan nem trivialis, ezert slotra kerdezunk
// ra, nem a hamis tomb sorrendjere.
live = [mkPad(0, SNES), mkPad(1, SNES)];
pads.update();
const P0 = live.find((p) => p.index === pads.info(0).index);
const P1 = live.find((p) => p.index === pads.info(1).index);
ok('a ket slot ket kulonbozo padot fog', P0 !== P1 && P0 && P1);

const IN = await import('../v3/src/input.js');
const fired = [];
IN.onKey((code) => fired.push(code));

P0.axes[0] = -1;                                   // 1. jatekos: balra
P1.axes[1] = 1;                                    // 2. jatekos: le
pads.update();
eq('getInput(0) bal', IN.getInput(0).left, true);
eq('getInput(1) le', IN.getInput(1).down, true);
eq('getInput(0) nem le', IN.getInput(0).down, false);
eq('menuben kulon betuk', fired.slice().sort(), ['ArrowDown', 'KeyA']);
P0.axes[0] = 0; P1.axes[1] = 0;
pads.update();

// A nem-irany gombok NEM billentyukoddá alakulnak: allapotonkent mast
// jelentenek, ezert nyers gombneven mennek tovabb az onPad()-en.
const padded = [];
IN.onPad((btn, slot) => padded.push(btn + '@' + slot));

fired.length = 0; padded.length = 0;
P0.buttons[9].pressed = true;                      // START
pads.update();
eq('START nem lesz billentyu', fired.slice(), []);
eq('START az onPad-en jon', padded.slice(), ['start@0']);
P0.buttons[9].pressed = false;
padded.length = 0;
P1.buttons[8].pressed = true;                      // SELECT a 2. paden
pads.update();
eq('SELECT slotszammal', padded.slice(), ['select@1']);
P1.buttons[8].pressed = false;
pads.update();

padded.length = 0;
P0.buttons[0].pressed = true;                      // X
P0.buttons[4].pressed = true;                      // L
pads.update();
eq('X es L is atmegy', padded.slice().sort(), ['l@0', 'x@0']);
P0.buttons[0].pressed = false; P0.buttons[4].pressed = false;
pads.update();

fired.length = 0; padded.length = 0;
IN.suspendInput(true);
P0.buttons[9].pressed = true;
P0.axes[0] = -1;
pads.update();
eq('kapu alatt nincs bemenet', [fired.slice(), padded.slice(), IN.getInput(0).left], [[], [], false]);
IN.suspendInput(false);
P0.buttons[9].pressed = false;
P0.axes[0] = 0;
pads.update();
