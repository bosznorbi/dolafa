...
export function drawPause(c, g) {
  ...
  drawText(c, goLabel() + ' = FOLYTATÁS', W / 2, 112, { align: 'center', color: PAL.ui });
  drawText(c, keyName('ESC', 'A') + ' = KILÉPÉS', W / 2, 126, { align: 'center', color: PAL.uiDim });
}
...
export function drawMatchEnd(c, g, S, time) {
  ...
  if (g.endless) {
    ...
    drawText(c, keyName('SHIFT+R', 'SELECT') + ' = STOP', W / 2, 158, { align: 'center', color: 'rgba(150,134,110,0.9)' });
    drawText(c, keyName('SPACE', 'A') + ' = MENÜ', W / 2, 168, { align: 'center', color: 'rgba(120,104,80,0.8)' });
    return;
  }
  if (t > 1.4) {
    // Szoros sorrend: elobb az ujra, aztan a vegtelen, vegul a kilepes.
    if (Math.floor(time * 2) % 2 === 0) {
      drawText(c, keyName('R', 'B') + ' = ÚJRA', W / 2, 136, { align: 'center', color: '#ffd257' });
    }
    drawText(c, keyName('SHIFT+R', 'SELECT') + ' = VÉGTELEN', W / 2, 146, { align: 'center', color: 'rgba(150,134,110,0.9)' });
    drawText(c, keyName('SPACE', 'A') + ' = MENÜ', W / 2, 156, { align: 'center', color: 'rgba(120,104,80,0.8)' });
  }
}
 ...
 */
/**
 * A kepernyon mindig annak a gombnak a neve all, ami tenylegesen a kezben
 * van. Ha be van dugva kontroller, a padgomb neve, kulonben a billentyu.
 */
function padOn() { return pads.count() > 0; }

function keyName(kb, pad) { return padOn() ? pad : kb; }

function goLabel() { return keyName('SPACE', 'START'); }
...
export function drawMenu(c, m, S, time) {
  ...
  if (Math.floor(time * 2) % 2 === 0) {
    drawText(c, goLabel(), W / 2, m.z ? 157 : 143,
  ...
  drawText(c, keyName('Z', 'SELECT'), 21, H - 14,
    Object.assign({ color: m.z ? '#ffe08a' : '#8a7a62' }, OL));
  ...
  drawText(c, keyName('M', 'L'), W - 13, H - 14,
    Object.assign({ color: muted ? '#8a7a62' : '#c8b89a' }, OL));
}
...
/** A racs szelessege. A magassagot a THEME_GRID hossza adja. */
const GRID_COLS = 3;

/** Melyik rekeszen allunk eppen. */
function gridCurSlot(m) {
  const cur = m.random ? THEME_GRID.indexOf(GRID_RANDOM) : THEME_GRID.indexOf(CURRENT.index);
  return cur < 0 ? THEME_GRID.indexOf(GRID_RANDOM) : cur;
}

/** Hova visz egy lepes. -1, ha arra nincs ervenyes rekesz. */
function gridTargetSlot(m, dx, dy) {
  const cur = gridCurSlot(m);
  const nx = (cur % GRID_COLS) + dx;
  const ny = ((cur / GRID_COLS) | 0) + dy;
  const rows = Math.ceil(THEME_GRID.length / GRID_COLS);
  if (nx < 0 || nx >= GRID_COLS || ny < 0 || ny >= rows) return -1;
  const slot = ny * GRID_COLS + nx;
  const v = THEME_GRID[slot];
  if (v === undefined || v === GRID_INFO) return -1;
  if (v !== GRID_RANDOM && v >= THEMES.length) return -1;
  return slot;
}

/**
 * Melyik padgomb visz melyik iranyba. Ugyanez a negy gomb all a racson is,
 * tehat a kepernyo maga tanitja meg a navigalast.
 */
const GRID_KEYS = [
  [-1, 0, 'Y'],
  [0, -1, 'X'],
...
/**
 * Lepes a palyavalaszto racson. Kontrollerrel nincs szamgomb, tehat a negy
 * jobb oldali gombbal kell tudni jarni a numpad-elrendezesen.
 *
 * A kivalasztott csempe MAGA a kurzor: ahova lepunk, az a palya azonnal
 * ervenybe is lep, pont ugy, mintha lenyomtak volna a szamat. Igy nincs
 * kulon "kurzor" es "kivalasztott" allapot, amit kulon kellene magyarazni.
 *
 * Az infogombot a kurzor ATLEPI: az a valltogomb dolga, es ha ra lehetne
 * lepni, minden athaladas be-ki kapcsolna a sugot.
 *
 * Visszaad: { theme: n } vagy { random: true }, ha van hova lepni; null, ha nincs.
 */
export function moveThemeGrid(m, dx, dy) {
  const slot = gridTargetSlot(m, dx, dy);
  if (slot < 0) return null;
  const v = THEME_GRID[slot];
  return v === GRID_RANDOM ? { random: true } : { theme: v };
}
...
function drawThemeStrip(c, m, time) {
  ...
  // Kontrollerrel a szamok nem mondanak semmit: a paden nincs szamgomb.
  // Helyettuk a NEGY SZOMSZED kapja meg annak a gombnak a betujet, amivel
  // odalephetunk, a tobbi rekesz csak egy potty, ahol eppen allunk, oda
  // pedig egy kurzor kerul. Ennyibol ki lehet talalni a navigalast.
  let padLab = null;
  if (padOn()) {
    padLab = {};
    for (const [dx, dy, ch] of GRID_KEYS) {
      const t = gridTargetSlot(m, dx, dy);
      if (t >= 0) padLab[t] = ch;
    }
    padLab[gridCurSlot(m)] = '@';         // itt allunk
  }
  ...
  for (let slot = 0; slot < THEME_GRID.length; slot++) {
    ...
    if (inf) {
      ...
      // Kontrolleren a jobb valltogomb kapcsolja a sugot, tehat az all rajta.
      drawText(c, padOn() ? 'R' : 'I', cx - 2, cy - 3, Object.assign({ color: col }, OL));
      continue;
    }
    ...
    if (!padLab) {
      ...
      // Kurzor: tomor rombusz. Nem betu, mert nincs olyan gomb, amivel
      // "ide" lehetne lepni: itt mar itt vagyunk.
      c.fillStyle = 'rgba(255,242,192,' + pulse.toFixed(2) + ')';
      const DIA = [[2, 1], [1, 3], [0, 5], [1, 3], [2, 1]];
      for (let r = 0; r < DIA.length; r++) {
        c.fillRect(bx + 3 + DIA[r][0], by + 8 + r, DIA[r][1], 1);
      }
    } else if (padLab[slot]) {
      drawText(c, padLab[slot], bx + 3, by + 7,
        Object.assign({ color: '#e2d2ac' }, OL));
    } else {
      // Tavolabbi rekesz: csak egy potty, hogy ne vonja el a figyelmet a
      // negy elerheto iranyrol.
      c.fillStyle = '#6a6254';
      c.fillRect(bx + 4, by + 9, 3, 2);
    }
  }
