...
import { initInput, onKey, onPad, suspendInput, pads } from './input.js';
...
// A menu es a zarokep muveletei kulon fuggvenyben allnak, mert KET helyrol
// hivodnak: a billentyuzetrol es a kontrollerrol. Egy allapotvaltasnak egy
// helyen kell lennie, kulonben a ket ut lassan szetcsuszik.
...
function setRandom(on) {
  ...
  setMusicTrack(on ? 'meglepetes' : THEMES[CURRENT.index].id);
  setHazardAmbience(on ? 'mystery' : THEMES[CURRENT.index].hazard);
  sfx.select();
}
...
function resume() {
  g.state = g.pausedFrom || 'playing';
  g.stateT = g.pausedT || 0;
}
...
onKey((code, shift) => {
  ...
  if (code === 'KeyR' && shift) { toggleEndless(); return; }
  ...
  if (g.state === 'menu') {
    ...
    if (code === 'KeyZ') { toggleZ(); return; }
    if (menu.z) {
      // I: a sugo-buborek. Egy felmondatban elmondja, mit csinal a csavar.
      if (code === 'KeyI') { toggleInfo(); return; }
      if (code === 'KeyR') { setRandom(!menu.random); return; }
      const d = digitOf(code);
      if (d !== null && d < THEMES.length) { pickTheme(d); return; }
    }
  ...
  if (g.state === 'matchend') {
    ...
    if (confirm || code === 'Escape') leaveMatch();
...
// ------------------------------------------------------------ kontrollerek
//
// Minden billentyus muveletnek van padgombja. Az IRANYOKAT az input.js mar
// elintezte (azok jatekosonkent kulon mennek), itt jon a tobbi gomb.
//
// A negy jobb oldali gomb a MENUBEN masodik iranykereszt: azzal lehet jarni
// a palyavalaszto numpad-racsot, mert szamgomb nincs a paden.
const GRID_STEP = {
  x: [0, -1],      // fel
  b: [0, 1],       // le
  y: [-1, 0],      // bal
...
onPad((btn) => {
  ...
  // A bal valgomb barhol nemit, ahogy az M billentyu.
  if (btn === 'l') { toggleMute(); return; }

  if (g.state === 'menu') {
    if (btn === 'start') { startMatch(menu.sides); return; }
    if (btn === 'select') { toggleZ(); return; }
    if (!menu.z) return;              // varazsmod nelkul nincs racs es nincs sugo
    if (btn === 'r') { toggleInfo(); return; }
    const step = GRID_STEP[btn];
    if (!step) return;
    const to = moveThemeGrid(menu, step[0], step[1]);
    if (!to) return;
    if (to.random) setRandom(true);
    else pickTheme(to.theme);
    return;
  }

  if (g.state === 'matchend') {
    if (btn === 'b') { startMatch(g.sides); return; }
    if (btn === 'select') { toggleEndless(); return; }
    if (btn === 'a') { leaveMatch(); return; }
    return;
  }

  if (g.state === 'paused') {
    if (btn === 'start') { resume(); return; }
    if (btn === 'a') { toMenu(); return; }
    if (btn === 'select') { toggleEndless(); return; }
    return;
  }

  if (g.state === 'playing' || g.state === 'countdown' || g.state === 'roundend') {
    if (btn === 'start') { pause(); return; }
    if (btn === 'select') { toggleEndless(); return; }
  }
});

// Toltokepernyo indulaskor: megvarjuk, amig mindket kontroller a helyere
// kerul, es kozben ki lehet probalni a gombjaikat. A jatek addig is fut es
// sotetitve latszik mogotte.
let padMode = false;
...
function openGate() {
  ...
  padGate({
    players: 2,
    title: 'DŐL A FA',
    subtitle: 'Két USB kontroller kell. Dugd be őket, és nyomj meg rajtuk egy gombot.',
    onClose: ({ keyboard }) => {
      padMode = !keyboard;
      suspendInput(false);
    },
...
// Ha jatek kozben kiesik egy kontroller, allunk meg, es jojjon vissza a kapu:
// igy nem kell ujratolteni a lapot amiatt, hogy valaki belerugott a kabelbe.
window.addEventListener('gamepaddisconnected', () => {
  if (!padMode || pads.count() >= 2) return;
  padMode = false;
  pause();
  openGate();
});
...
window.__timber = {
  g, S, menu, setTheme, THEMES, CURRENT,
  step: (dt) => update(dt),
  render: (t) => draw(t),
  startMatch,
  newRound,
