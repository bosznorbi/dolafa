# DŐL A FA - v3 (kontrollerrel)

Ez a **v2 másolata**, plusz **két USB SNES kontroller** támogatása. A játékmenet
és minden pályavilág változatlan.

A v1 (repó gyökere) és a v2 érintetlen marad. Ha valami elromlik itt, ott
ugyanaz a játék fut tovább.
...
Ugyanaz a szerver szolgálja ki mindet. Nincs `npm install`, nincs build.

## Kontrollerek

Induláskor egy **teljes képernyős töltőképernyő** fogad: megvárja, amíg mindkét
kontroller a helyére kerül, közben a játék sötétítve látszik mögötte.

Itt derül ki, hány pad van bedugva, melyik melyik játékosé, és hogy minden
gombjuk működik-e: az **éppen nyomott gomb élénkzöld**, a **már egyszer
kipróbált halványzöld**, és a képernyő számolja, hány van meg a 12-ből. Így
végig lehet nyomkodni a padet, és látszik, ha valamelyik gomb néma.

A játékot a **START nyomva tartása** indítja, nem egy koccintás. Így a második
játékost kiválasztó gombnyomás nem indítja el rögtön a meccset, és a START is
végigpróbálható gomb marad.

A töltőképernyő nem kényelmi funkció: a böngésző **addig nem is látja a padet,
amíg meg nem nyomnak rajta egy gombot**. Az első gombnyomás egyben a felismerés is.

### Gombkiosztás

Minden billentyűs műveletnek van padgombja, tehát a játék végig irányítható
kontrollerrel.

| Pad gomb | Menüben | Játék közben | Szünetben | Meccs végén |
|---|---|---|---|---|
| iránykereszt | fel-le: szereplő, jobb-bal: ember vagy bot | mozgás | | |
| X Y B A | pályaválasztó rács: fel, bal, le, jobb | | | |
| START | meccs indítása | szünet | folytatás | |
| SELECT | varázsmód (`Z`) | végtelen mód | végtelen mód | végtelen mód |
| A | | | vissza a menübe | vissza a menübe |
| B | | | | ÚJRA |
| L váll | némítás (`M`) | némítás | némítás | némítás |
| R váll | súgóbuborék (`I`) | | | |

A pályaválasztó a numpad-elrendezésű rács: a négy jobb oldali gomb második
iránykeresztként jár rajta, mert a paden nincs számgomb. Ahova lépsz, az a pálya
azonnal érvénybe is lép, ahogy a szám lenyomásakor.

**Bedugott kontrollernél a rács maga mutatja meg a navigálást.** A számok eltűnnek,
és helyettük az látszik, melyik gombbal hova lehet lépni:

```
  ·   X   ·          X = fel      A kurzoron egy rombusz áll,
  Y   ◆   A          Y = bal      a négy szomszédon a hozzá tartozó
  ·   B   ·          A = jobb     gomb betűje, a többin csak egy pötty.
  ·   ·  (R)         B = le
```

A súgó karikáján `R` áll, mert azt a jobb vállgomb kapcsolja. A kurzor átlépi:
ha rá lehetne lépni, minden áthaladás be-ki kapcsolná a súgót.

A képernyőn mindig annak a gombnak a neve áll, ami tényleg a kezedben van: ha be
van dugva kontroller, a padgomb neve, különben a billentyűé.

A billentyűzet **közben is él**: a kettő vagy kapcsolatban van, tehát gép mellől
is bele lehet szólni. Ha nincs kéznél kontroller, a töltőképernyő
`BILLENTYŰZETTEL` gombja átugorja az egészet.

Ha játék közben kiesik egy kontroller, a játék megáll, és visszajön a
töltőképernyő: nem kell újratölteni a lapot.

A padkezelést a `src/gamepad.js` és a `src/pad-gate.js` végzi.

> **Ez a két fájl máshonnan származik.** A forrásuk a
> [nitrogames](https://github.com/bosznorbi/nitrogames) repó
> (`starter/src/`), ez itt csak másolat. Ha ott változnak, ide kézzel kell
> átmásolni őket. Ugyanez vonatkozik a `.claude/skills/snes-kontroller`
> mappára is.
