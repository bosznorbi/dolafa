...
initInput(window);
// Erintes: hüvelykujj-konzol, koppintas, sarokgombok. Ugyanazokat a
// billentyukodokat kuldi, amiket az onKey() alabb kezel.
initErintes();
onKey((code, shift) => {
  initAudio();
...
function frame(now) {
  ...
  try {
    ...
    // Az erintes-reteg innen tudja, hol tart a jatek, es melyik oldalon ul
    // ember: jatekban csak ott van konzol. A menuben mindket oldal szabad,
    // mert ott dol el, ki jatszik. (mode 0 = JÁTÉKOS, lasd MENU_MODES.)
    {
      const sides = (g.state === 'menu' || !g.sides) ? menu.sides : g.sides;
      erintesBeallit({ state: g.state, emberek: [sides[0].mode === 0, sides[1].mode === 0] });
    }
    draw(clock);
  } catch (e) {
