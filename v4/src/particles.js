...
/**
 * Tuzijatek-robbanas: SUGARIRANYBAN szetrepulo szikrak, lassan hullo
 * csoval. A szinet a felszallt raketa adja, tehat mindegyik masra bomlik.
 */
export function burst(x, y, cols, n) {
  const list = cols && cols.length ? cols : PAL.fire;
  for (let i = 0; i < (n || 22); i++) {
    const a = (i / (n || 22)) * Math.PI * 2 + Math.random() * 0.3;
    const s = 34 + Math.random() * 44;
    add({
      x, y,
      vx: Math.cos(a) * s,
      vy: Math.sin(a) * s * 0.7,
      g: 52, life: 0.7 + Math.random() * 0.7, t: 0,
      col: list[(Math.random() * list.length) | 0],
      size: 1, shrink: false, fade: true,
    });
  }
  for (let i = 0; i < 6; i++) {          // feher mag a robbanas kozepen
    const a = Math.random() * Math.PI * 2;
    add({
      x, y,
      vx: Math.cos(a) * 22,
      vy: Math.sin(a) * 16,
      g: 30, life: 0.3 + Math.random() * 0.25, t: 0,
      col: '#fff8dc', size: 1, shrink: false, fade: true,
    });
  }
}
