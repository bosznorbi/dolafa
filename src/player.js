...
export function updatePlayer(p, dt, g) {
  ...
  const speed = CFG.speed * p.boost * (p.isBot ? (CFG.bot[p.difficulty] || CFG.bot.ugyes).speedMul : 1);
  const tvx = ax * speed;
  const tvy = ay * speed;
  const rate = (ax || ay ? CFG.accel : CFG.friction) * dt;
  ...
  p.chopping = false;
  const tree = nearestChoppable(p, g.trees);
  if (tree) {
    p.chopping = true;
...
export function resolveMelee(p0, p1) {
  ...
  att.meleeCd = CFG.meleeRate;
  // Nem fix ertek: harang-eloszlas, tehat a kozepes csapas a gyakori.
  const dmg = Math.round(bellRange(CFG.meleeMin, CFG.meleeMax));
  ...
  vic.kx = Math.cos(ang) * CFG.meleeKnock;
  vic.ky = Math.sin(ang) * CFG.meleeKnock * 0.7;
